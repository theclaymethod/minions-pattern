...
When the user provides a task example intended as a reusable precedent, determine its host, pattern, worker tier, adversarial mode, verifier tier, and rationale. Confirm any ambiguous mapping, then add a compact entry to `references/examples.md`. Treat examples as routing precedents, not immutable rules.
